...
export default function useWatchHistory(token: string): {
    ...
    const updateWatchHistory = useCallback(async (entry: WatchHistoryEntry) => {
        if (!token) return
    ...
    const getWatchHistoryItem = useCallback(async (imdb_id: string, season?: number, episode?: number) => {
        if (!token) return
        ...
        try {
            let res: Response
            if (season && episode) {
                res = await fetch(`${API_BASE}/me/watch-history/${imdb_id}?season=${season}&episode=${episode}`, {
                    headers: { Authorization: `Bearer ${token}` }
                })
            } else {
