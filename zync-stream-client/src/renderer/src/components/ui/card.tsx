...
function Card({ className, ...props }: React.ComponentProps<"div">): React.ReactElement {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-black text-white flex flex-col gap-6 rounded-2xl shadow-xl border border-white/15 hover:shadow-2xl transition-all duration-200 backdrop-blur-xl ring-1 ring-white/10 hover:ring-white/20",
        className
      )}
      {...props}
    />
